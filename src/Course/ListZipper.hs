...
findLeft p (ListZipper l a r) =
  if length ys == 0
    then MLZ Empty
    else MLZ (Full (ListZipper (drop 1 ys) (headOr a ys) ((reverse xs) ++ (a :. r))))
  where
    (xs, ys) = break p l
...
findRight p (ListZipper l a r) =
  if length ys == 0
    then MLZ Empty
    else MLZ (Full (ListZipper ((reverse xs) ++ (a :. Nil) ++ l) (headOr a ys) (drop 1 ys)))
  where
    (xs, ys) = break p r
...
moveLeftLoop (ListZipper Nil a r) = ListZipper ((tail . reverse) r ++ a :. Nil) (last r) Nil
...
moveRightLoop (ListZipper l a Nil) = ListZipper Nil (last l) (tail $ reverse (a :. l))
...
moveLeft (ListZipper (x :. xs) a r) = MLZ $ Full $ ListZipper xs x (a :. r)
...
moveRight (ListZipper l a (x :. xs)) = MLZ $ Full $ ListZipper (a :. l) x xs
...
swapLeft (ListZipper (x :. xs) a r) = MLZ $ Full $ ListZipper (a :. xs) x r
...
swapRight (ListZipper l a (x :. xs)) = MLZ $ Full $ ListZipper l x (a :. xs)
...
deletePullRight (ListZipper l _ (r :. rs)) = MLZ $ Full (ListZipper l r rs)
...
insertPushLeft ::
  ...
  ListZipper a ->
  ListZipper a
insertPushLeft n (ListZipper l a r) = ListZipper (a :. l) n r
...
insertPushRight ::
  ...
  ListZipper a ->
  ListZipper a
insertPushRight n (ListZipper l a r) = ListZipper l n (a :. r)
...
instance Applicative ListZipper where
  ...
  (<*>) (ListZipper lf af rf) (ListZipper l a r) = ListZipper left a' right
    where
      left = zipWith id lf l
      right = zipWith id rf r
      a' = af a
...
instance Applicative MaybeListZipper where
  ...
  (<*>) _ (MLZ Empty)                 = MLZ Empty
  (<*>) (MLZ (Full f)) (MLZ (Full a)) = MLZ $ Full (f <*> a)
...
instance Extend ListZipper where
  (<<=) :: (ListZipper a -> b) -> ListZipper a -> ListZipper b
  (<<=) f lz@(ListZipper l _ r) = ListZipper lefts (f lz) rights
    where
      lefts = gen moveLeft lz
      rights = gen moveRight lz
      gen mov =
        unfoldr
          ( \l -> case toOptional $ mov l of
              Full l' -> Full (f l', l')
              Empty   -> Empty
          )
...
instance Traversable ListZipper where
  ...
  traverse f (ListZipper l a r) = ListZipper <$> lefts <*> f a <*> rights
    where
      lefts = reverse <$> traverse f (reverse l)
      rights = traverse f r
