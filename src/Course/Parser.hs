...
character = P pChar
  where
    pChar Nil       = UnexpectedEof
    pChar (x :. xs) = Result xs x
...
satisfy ::
  (Char -> Bool) ->
  Parser Char
satisfy p = character >>= (\a -> if p a then valueParser a else unexpectedCharParser a)
...
sequenceParser ::
  List (Parser a) ->
  Parser (List a)
sequenceParser = foldRight go (pure Nil)
  where
    go x xs = x .:. xs
...
smokerParser =
  (is 'y' ||| is 'n')
    >>= ( \c -> case c of
            'y' -> valueParser True
            'n' -> valueParser False
            a   -> constantParser $ UnexpectedChar a
        )
...
personParser = do
  ...
  first <- firstNameParser <* spaces1
  last <- surnameParser <* spaces1
  smoker <- smokerParser <* spaces1
  phone <- phoneParser
  pure $ Person age first last smoker phone
